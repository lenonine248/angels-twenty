...
//
// **基準はステージごとに既定搭載を見て置いてある**（各ステージのコメントを読むこと）。
// ◎ が既定搭載より低いステージもある — IRON UMBRELLA は
// 「無誘導爆弾（0P）に積み替えて低空で入れば節約できる」という設計。
//
// §32.6 で一度、既定搭載＋予算の15%/40% に一括で引き直したが**戻した**。
// 各ステージのコメントを読むと、値は意図して置かれていた。
// 節約が苦しくなるのは基準のせいではなく、**帰投後の補給が黙ってポイントを
// 使うから**だった。そちらを見えるようにした（§32.6）。
//
// 各軸 [◎の上限, ○の上限]。○の上限も超えたら △。
