...
export const TUTORIALS = [
  ...
  // ================================================================ 4
  {
    ...
    steps: [
      { text: '機体を選択し、下のパネルの「レーダー」の欄を見る',
        note: '自動／常時ON／常時OFF の3つ。右に「放射中」か「沈黙」かが出ます。'
  ...
  // ================================================================ 5
  {
    ...
    enemy: {
      ...
      // どちらも生きている自機を前提にしている）。
      //
      // 撃たれたときの話は「回避とデコイ」が丸ごと受け持っている。
      // **ここで教えるのは撃つ距離の選び方であって、撃たれ方ではない。**
      aircraft: [dummyFighter('BANDIT 1', 30000, 24000, 5500)],
      ground: [],
    },
    steps: [
      ...
      { text: 'AAM-M を発射する',
        note: '敵を右クリックすると、指定した兵装での射撃指示になります。'
          ...
          + '**射程に入っても、機首が敵の方を向くまでは撃ちません。** '
          + 'ミサイルは機首の向きに出るので、そこから曲がるぶんの時間と力を失います。'
          + '許される角度は**弾が飛ぶ時間しだい** — '
          + '遠ければ横を向いていても撃てますが、近いほど正面を向くまで待ちます',
  ...
  // ================================================================ 6
  {
    ...
    steps: [
      { text: '機体を選択し、デコイの残数を確かめる',
        note: `F-1 はフレア${AC('F-1').flares}・チャフ${AC('F-1').chaff}。**働きが違います** — `
    ...
    steps: [
      ...
      { text: '搭載を積み替える',
        note: '下のパネルで兵装を足す／降ろす。足すと兵装ポイントが減り、降ろすと戻ります。\n'
          + '**枠には太さがあります。** 「中2/4 小1/2」のような表示が'
          + 'いま使っている枠で、太い兵装（AAM-A・AGM・ARM・BOMB・増槽）は'
          + '**中型の枠にしか載りません**。細い空対空弾（AAM-S・AAM-M）は'
          + '小型にも、余っている中型にも載ります。\n'
          + '**赤くなったら載せ切れていません** — 数は足りていても、'
          + '中型が埋まっていれば対地兵装はもう積めません',
        done: 'loadout' },
  ...
  // ================================================================ 兵装3
  {
    ...
    steps: [
      ...
      { text: 'AAM-M を発射する',
        note: '兵装を指定した状態で敵を右クリックすると射撃指示になります。'
          + '射程(20km)の内側に入っていること、'
          + '**そして機首が敵の方を向いていること**が要ります —— '
          + 'ミサイルは機首の向きに出るので、曲がり切れない角度からは撃ちません',
        done: 'fire', when: (d) => d.weapon === 'AAM-M',
        check: (ctx) => mine(ctx).length > 0
          && mine(ctx).every((u) => !u.loadout.includes('AAM-M')) },
  ...
  // ================================================================ 兵装4
  {
    ...
    steps: [
      ...
      { text: '下のパネルで使用兵装に AAM-A を指定する',
        note: '**中型の枠を1本、積載を2つぶん使います。** '
          + 'F-1 は中型2本・積載4なので、どちらから見ても2発が上限です。'
          + 'その2本を使うと、対地兵装も増槽も載りません',
  ...
  // ================================================================ 兵装5
  {
    ...
    steps: [
      ...
      { text: '下のパネルで使用兵装に AGM を指定する',
        note: 'コスト4。**中型の枠を1本、積載を2つぶん**使います。'
          + 'A-3 は中型4本・積載6なので3発まで。'
          + '対地兵装はどれも太いので、**積める数を決めるのはたいてい中型の本数**です',
  ...
  // ================================================================ 兵装6
  {
    ...
    steps: [
      ...
      { text: '下のパネルで使用兵装に ARM を指定する',
        note: 'コスト6。**中型の枠を1本、積載を2つぶん**使います。'
          + '高価なので外したくない兵装です',
  ...
  // ================================================================ 兵装7
  {
    ...
    steps: [
      ...
      { text: '下のパネルで使用兵装に BOMB を指定する',
        note: 'コスト0。兵装ポイントを一切使いません。'
          + 'ただし**太いので中型の枠が要ります** — '
          + '重さは1つぶんでも、載る場所は限られます',
  ...
  // ================================================================ 兵装8
  {
    ...
    name: 'TANK 増槽',
    title: '足を伸ばす',
    brief: '武器ではありませんが、中型の枠を1本使う搭載品です。燃料が40%増えます。\n'
    ...
    steps: [
      ...
      { text: '整備を終えて、燃料の持ち時間が増えたことを確認する',
        note: '燃料 +40%。中型の枠が1本埋まり、機動性もわずかに落ちます',
    ...
    steps: [
      ...
      { text: 'ANVIL 1 のAIモードを「対地攻撃」にする',
        note: '見えている地上目標へ進撃します。'
          ...
          + '逆探知に映らず、ARM も誘導しません。射程6km・射高5,000m と短いぶん、'
          + '避けるなら**上を通る**ことになります',
