...
export const WEAPONS = {
  ...
  'AAM-A': {
    ...
    maxG: 25,                 // 最大G（§34.2）
    // **慣性飛行が長い**（§89）。値段を下げる代わりに性能を上げた ——
    // プレイヤーの判断。既定の 35 秒では `drag = 速度 ÷ coastTime` から
    // 実効到達が **約17.5km** で、AI の平均発射距離 16.5km と同じところに
    // 崖があり、**外れの38%が失速**だった。45 秒で失速は 4% に落ちる。
    //
    // `[実測]` 前3面・同じ種18・A+M+S+S を積んで同じ出撃で比べる:
    //
    ...
    // | 55 | 2% | 0.652 | 486m |
    //
    // **55 は行き過ぎ**（1発あたり AAM-M の4倍になり、選ぶ理由が無くなる側へ振れる）。
    // 表記射程 20km は据え置き —— **20km と書いてある弾が本当に 20km 届く**ようになる、
    // という差になる（AAM-M の 20km は依然として約17.5kmまでの意味）。
    coastTime: 45,
    // **終末だけ余分に引ける**（§89）。中途を慣性で飛ぶあいだ舵を使っていないので、
    // 残した余力を最後に出す、という理屈。**照射され続ける弾には持たせられない。**
    // 外れの最接近が 1,685m → 590m に縮む（＝至近弾が命中に変わる）。
    terminalG: 32,
    // **データリンク**（§89.7）。中途の位置を**陣営のどの機体からでも**もらえる。
    // 開くのは精度ではなく**撃てる場面**で、
    //
    //   ・**レーダーを切ったまま撃てる**（逆探知に映らずに撃つ）
    //   ・撃った本人が落とされても、僚機が見ていれば弾が生き続ける
    //
    // **扇そのものは開かない。** 黙っていても機首は向ける ——
    // 扇ごと開けると AI が縦の外へ撃ち始めて損失が倍になった（§89.7 で実測）。
    //
    // 中途の精度そのものは**測っても動かない**（1発の撃墜 0.393 → 0.380）——
    // 更新を1回落としても終末シーカーが穴を埋めるため。
    // **値打ちは「撃てるかどうか」の側にある。**
    datalink: true,
    fireAndForget: true,
    ...
    //
    // **復活させても意味が無いことを測った**（§89）。
    // 妨害の効きを `1 - decoyResist` で縮める形で仮に効かせると、
    // 1発の撃墜は 0.393 → 0.407（+3.6%）しか動かない。
    // 強い妨害を受けた弾は AAM-A の25%あり、そのうち命中は0発 —— だが
    // **妨害されている弾は同時にビーム機動を受けている**ので、
    // 外れている原因は妨害ではなく幾何のほうだった。
    // 残すなら「効かせ方」を別に作ること。0.85 という数字に意味はまだ無い。
    decoyResist: 0.85,
    damage: 200,
    rearmSeconds: 30,
    ...
    desc: '僚機のレーダーでも導ける。自機のレーダーを切ったままでも撃てる。表記射程まで本当に届く',
  },
