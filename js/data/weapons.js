...
export const WEAPONS = {
  ...
  'AGM': {
    ...
    rearmSeconds: 25,
    desc: '対地ミサイル。どの高度からでも撃てるが、高度を上げても射程はあまり伸びない',
  },
  ...
  'ARM': {
    ...
    // **高度による射程の伸びを絞る**（§43）。
    //
    // 8,500m（`ARM_STANDOFF_ALT`）から撃つと実効射程が 26.7km と出て、
    // AI は 22.7km で発射する。ところが**実際に届くのは約21km**で、
    // 1.2〜1.7km 手前で力尽きていた（実測 6/6）。
    // 0.55 にすると発射上限が約20.9km になり、飛翔距離の内側に収まる。
    //
