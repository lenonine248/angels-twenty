...
export class CloudField {
  ...
  /**
   * 経過秒から直に位置を決める（リプレイ用・§23.7）。
   *
   * **`advance` と分けてある。** 向こうは1刻みずつ足し込む形で、
   * 18種のベンチはその足し方で測った値。ここを `v × t` に置き換えると
   * **最後の桁がずれて、長い戦闘では結末が変わりうる。**
   * 触らないほうの道を残したまま、**飛ばせる側**をこちらに足す
   * （リプレイは時刻を飛び越えるので、足し込みでは追えない）。
   */
  setElapsed(sec) {
    if (!this.blobs.length) return;
    this.offset.x = this.vx * sec;
    this.offset.z = this.vz * sec;
  }
